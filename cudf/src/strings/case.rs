...
impl Column {
    ...
    pub fn str_to_upper(&self) -> Result<Column> {
        let result = cudf_cxx::strings::case::ffi::str_to_upper(&self.inner)
    ...
    pub fn str_to_lower(&self) -> Result<Column> {
        let result = cudf_cxx::strings::case::ffi::str_to_lower(&self.inner)
            .map_err(CudfError::from_cxx)?;
        Ok(Column { inner: result })
    }
    ...
    pub fn str_swapcase(&self) -> Result<Column> {
        let result = cudf_cxx::strings::case::ffi::str_swapcase(&self.inner)
            .map_err(CudfError::from_cxx)?;
        Ok(Column { inner: result })
    }
}
