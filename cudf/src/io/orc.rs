...
impl OrcReader {
    ...
    pub fn read(self) -> Result<Table> {
        let raw = cudf_cxx::io::orc::ffi::read_orc(
            &self.path,
            &self.columns,
            self.skip_rows,
            self.num_rows,
    ...
    /// Read an ORC file, returning both the table and column names
    /// from the file schema metadata.
    pub fn read_with_metadata(self) -> Result<TableWithMetadata> {
        let raw = cudf_cxx::io::orc::ffi::read_orc_with_metadata(
            &self.path,
            &self.columns,
            self.skip_rows,
            self.num_rows,
...
impl<'a> OrcWriter<'a> {
    ...
    pub fn write(self) -> Result<()> {
        cudf_cxx::io::orc::ffi::write_orc(&self.table.inner, &self.path, self.compression as i32)
            .map_err(CudfError::from_cxx)
    }
}

pub fn read_orc(path: impl Into<String>) -> Result<Table> {
    OrcReader::new(path).read()
}
pub fn write_orc(table: &Table, path: impl Into<String>) -> Result<()> {
    OrcWriter::new(table, path).write()
}
